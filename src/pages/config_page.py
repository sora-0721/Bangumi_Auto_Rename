...
class ConfigPage(ui.dialog):
    ...
    def __init__(self) -> None:
        ...
        with self, ui.card().style(_s).classes('flex'):
            ...
            # 基础配置
            ui.label('基础配置').style('font-size: 16px; font-weight: bold; margin-top: 10px;')
            basic_configs = ['api_key', 'bangumi_path', 'movie_path', 'anime_path', 'anime_movie_path', 'mode', 'docker_mnt']
            ...
            # AI配置
            ui.label('AI识别配置').style('font-size: 16px; font-weight: bold; margin-top: 10px;')
            ai_configs = ['ai_enabled', 'ai_api_key', 'ai_base_url', 'ai_model', 'ai_confidence_threshold']
                ...
                with ui.row(wrap=False, align_items='baseline') as row:
                    ...
                    if cn == 'mode':
                        ...
                        tg = RedToogle(
                            ...
                            on_change=lambda e, c=cn: self._change(c, e.value == '启用'),
                        )
                        tg.style('font-size: 10px')
                        tg.classes('flex no-wrap w-full')
    ...
    def _handle_ok(self):
        for cn in self.config.__dict__:
            cm.set_config(cn, getattr(self.config, cn))
