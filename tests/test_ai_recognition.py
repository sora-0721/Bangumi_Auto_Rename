...
from src.rename.cleaner import remove_tag, remove_season, remove_episode, divide_by_year
...
class AIRecognitionTester:
    ...
    def manual_mode(self, path: Path):
        ...
        # 生成prompt
        prompt = self.ai_client._build_analysis_prompt(tv_info, file_analysis, None)
        ...
        ai_result = self.ai_client._extract_and_validate_json(llm_response)
        ...
        if ai_result:
            ...
            if ai_result.season_mapping:
                print(f"🗂️ 季度映射: {ai_result.season_mapping}")
            
            print(f"📋 映射数量: {len(ai_result.mapping)}")
            
            for i, mapping in enumerate(ai_result.mapping, 1):
                print(f"  {i}. {mapping.local_file}")
                print(f"     -> S{mapping.tmdb_season:02d}E{mapping.tmdb_episode:02d}")
                print(f"     类型: {mapping.episode_type}, 置信度: {mapping.confidence:.2f}")
            
            if ai_result.special_notes:
                print(f"📝 特殊说明: {ai_result.special_notes}")
    ...
    def auto_mode(self, path: Path):
        ...
        if not self.ai_client.is_available():
            ...
            print(f"  - API密钥: {'已配置' if cm.get_config('ai_api_key') else '未配置'}")
        ...
        # 调用AI分析
        print("\n🤖 调用AI分析...")
        ai_result = self.ai_client.analyze_episode_mapping(tv_info, file_analysis, None)
        ...
        print("✅ AI分析完成")
        print(f"📊 总体置信度: {ai_result.confidence:.2f}")
        print(f"💭 分析理由: {ai_result.reason}")
        
        # 显示季度映射
        if ai_result.season_mapping:
            print("\n🗂️ 季度映射:")
            for local_season, tmdb_seasons in ai_result.season_mapping.items():
                if isinstance(tmdb_seasons, list):
                    seasons_str = ", ".join(map(str, tmdb_seasons))
                    print(f"  {local_season} -> TMDB季度: [{seasons_str}]")
                else:
                    print(f"  {local_season} -> TMDB季度: {tmdb_seasons}")
        
        # 显示文件映射
        print(f"\n📋 文件映射 ({len(ai_result.mapping)}个):")
        for i, mapping in enumerate(ai_result.mapping, 1):
            confidence_icon = "🟢" if mapping.confidence >= 0.8 else "🟡" if mapping.confidence >= 0.6 else "🔴"
            type_icon = {"regular": "📺", "special": "⭐", "ova": "🎬", "movie": "🎭"}.get(mapping.episode_type, "❓")
            
            print(f"  {i}. {mapping.local_file}")
            print(f"     -> S{mapping.tmdb_season:02d}E{mapping.tmdb_episode:02d} {type_icon} {mapping.episode_type}")
            print(f"     {confidence_icon} 置信度: {mapping.confidence:.2f}")
        
        if ai_result.special_notes:
            print(f"\n📝 特殊说明: {ai_result.special_notes}")
        ...
        for mapping in ai_result.mapping:
            ...
            for video_file in video_files:
                if mapping.local_file in video_file.name:
                    source_file = video_file
                    break
            
            if not source_file:
                print(f"❌ 未找到文件: {mapping.local_file}")
                continue
        ...
        # 置信度分析
        low_confidence_count = sum(1 for m in ai_result.mapping if m.confidence < self.ai_client.confidence_threshold)
